...
func (cm *CertificateManager) SignLeafCert(hosts []string) ([]byte, interface{}, error) {
	...
	// An IP entry belongs in IPAddresses, not DNSNames — x509 rejects IP
	// strings as dNSName SANs. Reached when an SNI-less client hits a
	// transparent (address-only) flow.
	var dnsNames []string
	for _, h := range hosts {
