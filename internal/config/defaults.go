...
const DefaultConfigTOML = `# 这是Snirect的配置文件，保存到程序 ⚠工作目录⚠ 后重启程序生效。
...
const DefaultRulesTOML = `# 这是Snirect的规则（除路由规则）文件。
# This is the rule (except routing rules) file of Snirect.
...
# 你也可以通过将配置文件复制到工作目录下的 rules 文件夹中以达到向 Snirect 添加规则的目的。
# You can also copy the rules configuration file to the rules directory under working directory to add rules to Snirect.
