...
func TestPreferenceCacheBasic(t *testing.T) {
	...
	cache.set("test", "1.2.3.4", 5*time.Second)
...
func TestPreferenceCacheEviction(t *testing.T) {
	...
	cache.set("a", "1.1.1.1", time.Minute)
	cache.set("b", "2.2.2.2", time.Minute)
	cache.set("c", "3.3.3.3", time.Minute) // evicts one
...
func TestPreferenceCacheConcurrent(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			...
			cache.set(host, "127.0.0.1", time.Second)
		}()
	}
	wg.Wait()
}
...
func BenchmarkPreferenceCacheSetGet(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		cache.set(host, "127.0.0.1", time.Minute)
		_, _ = cache.get(host)
	}
}
...
// TestResolver_Resolve_BracketedIPv6HostRule is the regression guard for Hosts
// entries written as "[2001:db8::1]". net.ParseIP rejects brackets, so without
// stripping them Resolve treated the literal as a hostname and DNS-failed
// every IPv6 Hosts rule (behance/claude/netflix/spotify/…).
func TestResolver_Resolve_BracketedIPv6HostRule(t *testing.T) {
	baseRules := ruleslib.NewRules()
	baseRules.Hosts["*behance.net"] = "[2001:67c:2960:6464::9765:41c5]"
	baseRules.Init()
	r := &Resolver{
		Config:    &config.Config{},
		Rules:     baseRules,
	...
	ip, err := r.Resolve(context.Background(), "behance.net", nil)
	if err != nil {
		t.Fatalf("Resolve error: %v", err)
	}
	want := "2001:67c:2960:6464::9765:41c5"
	if ip != want {
		t.Errorf("got %q, want bare IPv6 %q (brackets stripped)", ip, want)
	}

	// Bracketed form on the input host itself should also work.
	ip, err = r.Resolve(context.Background(), "[2001:db8::1]", nil)
	if err != nil {
		t.Fatalf("Resolve bracketed input: %v", err)
	}
	if ip != "2001:db8::1" {
		t.Errorf("got %q, want 2001:db8::1", ip)
	}
}
...
func TestResolver_getECS_AutoMode(t *testing.T) {
	...
	// auto mode keeps reduced subnet precision
	if e.SourceNetmask != 24 {
		t.Errorf("expected netmask 24, got %d", e.SourceNetmask)
	}
}
...
func TestResolver_Close(t *testing.T) {
	r := NewResolver(&config.Config{}, (*ruleslib.Rules)(nil))
...
func TestNewBackend_Parsing(t *testing.T) {
	...
	wrappedRules := ruleslib.NewRules()
	b := newBackend(cfg, wrappedRules, nil)
	if b == nil {
	...
	b2 := newBackend(cfg2, ruleslib.NewRules(), nil)
	if b2 != nil {
...
func TestDoHUpstreamExchange_UnpackError(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/dns-message")
		_, _ = w.Write([]byte("invalid dns data"))
	}))
	defer ts.Close()
